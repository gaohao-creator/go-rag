...
func (h *Handler) Retrieve(c *gin.Context) {
	...
	chunks, err := h.retriever.Retrieve(c.Request.Context(), service.RetrieveInput{Question: req.Question, TopK: req.TopK, Score: req.Score, KnowledgeName: req.KnowledgeName})
	if err != nil {
		h.writeServiceError(c, err)
		return
	}
	webmiddleware.WriteOK(c, dto.RetrieveResponse{Document: chunks})
}
